    ...
    public class SendRequestHelper : ISendRequestHelper
    {
        ...
        public async Task<ResultResponse<PingDto>> PingRequestAsync(string ipAddress)
        {
            try
            {
                ...
                return new ResultResponse<PingDto>() { Success = result.SuccessPercent >= 50, Value = result , Message = "Ping Tests was Successfull" };
            }
            catch (Exception ex)
            {
                return new ResultResponse<PingDto>() { Success = false, Message = $"Ping test failed: {ex.Message}" };
            }
        }
        ...
        public async Task<ResultResponse> MakeHttpRequestAsync(string url)
        {
            try
            {
                ...
                {
                    HttpStatusCode.OK => new ResultResponse() { Success = response.IsSuccessStatusCode, Message = "Ok (200)" },
